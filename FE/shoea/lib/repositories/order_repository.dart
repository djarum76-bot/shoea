import 'dart:io';

import 'package:dio/dio.dart';
import 'package:shoea/models/order_model.dart';
import 'package:shoea/service/internet_service.dart';
import 'package:shoea/service/storage_service.dart';
import 'package:shoea/utils/constants.dart';

class OrderRepository{
  Future<void> addToOrder(int shoeID, String destinationCityID, int price, String etd, DateTime now)async{
    try{
      FormData formData = FormData.fromMap({
        Constants.shoeID : shoeID,
        Constants.originCityID : "32",
        Constants.destinationCityID : destinationCityID,
        Constants.price : price,
        Constants.etd : etd,
        Constants.createdAt : now.toIso8601String()
      });

      await InternetService.dio.post("/auth/add-to-order",
        data: formData,
        options: Options(
          headers: {
            Constants.accept : Constants.appJson,
            HttpHeaders.authorizationHeader : "${Constants.bearer} ${StorageService.box.read(Constants.token)}"
          }
        )
      );
    }catch(e){
      throw Exception(e);
    }
  }

  Future<List<OrderModel>> getAllOrder()async{
    try{
      final response = await InternetService.dio.get('/auth/get-all-order',
        options: Options(
          headers: {
            Constants.accept : Constants.appJson,
            HttpHeaders.authorizationHeader : "${Constants.bearer} ${StorageService.box.read(Constants.token)}"
          }
        )
      );

      if(response.statusCode == 200){
        final datas = response.data as List;
        final list = datas.map((data) => OrderModel.fromJson(data)).toList();
        return list;
      }else{
        throw Exception();
      }
    }catch(e){
      throw Exception(e);
    }
  }
}